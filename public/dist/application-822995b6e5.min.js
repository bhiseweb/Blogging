...
!function(){"use strict";function menuService(){function addMenu(menuId,options){return options=options||{},service.menus[menuId]={roles:options.roles||service.defaultRoles,items:options.items||[],shouldRender:shouldRender},service.menus[menuId]}function addMenuItem(menuId,options){return service.validateMenuExistence(menuId),options=options||{},service.menus[menuId].items.push({title:options.title||"",state:options.state||"",type:options.type||"item",class:options.class,roles:null===options.roles||void 0===options.roles?service.defaultRoles:options.roles,position:options.position||0,items:[],shouldRender:shouldRender}),options.items&&options.items.forEach(function(subMenuItem){service.addSubMenuItem(menuId,options.state,subMenuItem)}),service.menus[menuId]}function addSubMenuItem(menuId,parentItemState,options){return options=options||{},service.validateMenuExistence(menuId),service.menus[menuId].items.filter(function(item){return item.state===parentItemState}).forEach(function(item){item.items.push({title:options.title||"",state:options.state||"",params:options.params||{},roles:null===options.roles||void 0===options.roles?item.roles:options.roles,position:options.position||0,shouldRender:shouldRender})}),service.menus[menuId]}function getMenu(menuId){return service.validateMenuExistence(menuId),service.menus[menuId]}function removeMenu(menuId){service.validateMenuExistence(menuId),delete service.menus[menuId]}function removeMenuItem(menuId,menuItemState){return service.validateMenuExistence(menuId),service.menus[menuId].items=service.menus[menuId].items.filter(function(item){return item.state!==menuItemState}),service.menus[menuId]}function removeSubMenuItem(menuId,subMenuItemState){return service.validateMenuExistence(menuId),service.menus[menuId].items.forEach(function(parentMenuItem){parentMenuItem.items=parentMenuItem.items.filter(function(subMenuItem){return subMenuItem.state!==subMenuItemState})}),service.menus[menuId]}function validateMenuExistence(menuId){if(!menuId||!menuId.length)throw new Error("MenuId was not provided");if(!service.menus[menuId])throw new Error("Menu does not exist");return!0}var shouldRender,service={addMenu:addMenu,addMenuItem:addMenuItem,addSubMenuItem:addSubMenuItem,defaultRoles:["user","admin"],getMenu:getMenu,menus:{},removeMenu:removeMenu,removeMenuItem:removeMenuItem,removeSubMenuItem:removeSubMenuItem,validateMenuExistence:validateMenuExistence};return function(){shouldRender=function(user){return-1!==this.roles.indexOf("*")||!!user&&user.roles.filter(function(userRole){return-1!==this.roles.indexOf(userRole)},this).length>0},addMenu("topbar",{roles:["*"]})}(),service}angular.module("core").factory("menuService",menuService)}();
